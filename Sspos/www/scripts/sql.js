sql = (function() {
	...
	ret.createTable = function(){
		db.transaction(function(tx){
	        ...
	        tx.executeSql('create table if not exists product (sku text primary key,title text,price text,sold_out text,nid text,image text,imageurl text,start_time text,end_time text)');
	        //创建订单表
	        tx.executeSql('DROP TABLE IF EXISTS orderinfo');
	        tx.executeSql('create table if not exists orderinfo (orderid text primary key,payamount text,paystatus text,paytype text,dicountamount text,productlist text,addtime text)');
		},function(e){
			console.log("创建数据表失败");
		})
	};
			...
			if(result.state !=1||result.data == ""){
				...
				db.transaction(function(tx){
					...
					for(var i=0;i<products.length;i++){
                        ...
                        tx.executeSql('insert or replace into product (sku,title,price,sold_out,nid,image,imageurl,start_time,end_time) values(?,?,?,?,?,?,?,?,?)',
                                [product.sku, product.title, product.price, product.sold_out, nid, ret.getThumbNameByUrl(my_image), my_image,timelist[0],timelist[timelist.length-1]],
                                function(tx,res){
